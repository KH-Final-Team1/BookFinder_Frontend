...
export const fetchBookInfo = async (isbn) => {
  try {
    const response = await axios.get(
        `${API_URL}/srchDtlList?authKey=${AUTH_KEY}&isbn13=${isbn}&format=json`);
...
export const getLibraryList = async (isbn, pageNo) => {
  try {
    const response = await axios.get(`${API_URL}/libSrchByBook`, {
      params: {
        authKey: AUTH_KEY,
        isbn: isbn,
        region: "11",
...
export const loanBook = async (libCode, isbn) => {
  try {
    const response = await axios.get(`${API_URL}/bookExist`, {
      params: {
        authKey: AUTH_KEY,
        libCode: libCode,
        isbn13: isbn,
